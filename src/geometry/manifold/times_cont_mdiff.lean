...
lemma times_cont_mdiff_on_ext_chart_at :
  times_cont_mdiff_on I 𝓘(𝕜, E) n (ext_chart_at I x) (ext_chart_at I x).source :=
...
/-- The `g ∘ f` is `C^n` within `s` at `x` if `g` is `C^n` at `f x` and
...
lemma times_cont_mdiff_of_support {f : M → F} (hfs : closure (support f) ⊆ s) (hs : is_open s)
  (h : times_cont_mdiff_on I 𝓘(𝕜, F) n f s) :
  times_cont_mdiff I 𝓘(𝕜, F) n f :=
begin
  intro x,
  by_cases hx : x ∈ closure (support f),
  { exact (h x (hfs hx)).times_cont_mdiff_at (mem_nhds_sets hs (hfs hx)) },
